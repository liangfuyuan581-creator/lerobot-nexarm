...
class _NormalizationMixin:
    ...
    **Stats Override Preservation:**
    ...
    _cached_device: torch.device | None = field(default=None, init=False, repr=False)
    _cached_dtype: torch.dtype | None = field(default=None, init=False, repr=False)
    ...
    def _apply_transform(
        ...
        # Cache the last seen device/dtype to avoid re-converting all stats every frame.
        if self._tensor_stats and key in self._tensor_stats:
            first_stat = next(iter(self._tensor_stats[key].values()))
            if (first_stat.device != tensor.device or first_stat.dtype != tensor.dtype) and (
                tensor.device != self._cached_device or tensor.dtype != self._cached_dtype
            ):
                self._cached_device = tensor.device
                self._cached_dtype = tensor.dtype
